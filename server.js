...
app.post('/api/submit', upload.single('designImage'), async (req, res) => {
	const { carpetName, length, width, yourName, yourEmail, yourMobileNumber } =
		req.body;
...
app.post(
	...
	async (req, res) => {
		try {
			...
			productData.frontImage = req.files['frontImage'][0].filename.replace(
				/\\/g,
				'/'
			);
			productData.backImage = req.files['backImage'][0].filename.replace(
				/\\/g,
				'/'
			);
			productData.sideImage = req.files['sideImage'][0].filename.replace(
				/\\/g,
				'/'
			);
